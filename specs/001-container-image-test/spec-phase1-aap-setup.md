...
3. **Given** AAP instance failed to start, **When** the setup script detects failure, **Then** it fails with clear error message showing AAP logs
...
- What happens when AAP fails health check?
  - Setup should retry health check 3 times, then fail with diagnostic info
...
- **FR-003**: System MUST start AAP instance on correct port (44925 for AAP 2.5, 44926 for AAP 2.6)
- **FR-004**: System MUST retrieve AAP admin password automatically from aap-dev output
- **FR-005**: System MUST retrieve AAP URL automatically from aap-dev output
- **FR-006**: System MUST validate AAP instance is running via health check endpoint (/api/v2/ping/)
- **FR-007**: System MUST wait for AAP to be fully ready (timeout 600 seconds)
- **FR-008**: System MUST provide structured logging with [PHASE] markers for AAP setup steps
- **FR-009**: System MUST support --skip-aap mode to reuse existing AAP instance
...
- **aap-dev Repository**: GitHub repository containing AAP development environment setup
...
|------|--------|------------|
| AAP-dev breaks or becomes unavailable | High - setup cannot work | Pin aap-dev to specific commit; document commit hash in script |
